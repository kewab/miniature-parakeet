...
- Every generated/downloaded image file must pass binary image validation (JPEG/PNG/WebP/GIF magic bytes).
- Reject files that are JSON/text even if extension is `.jpg`.
- If validation fails, auto-retry at least 2 times with backoff, then continue fallback chain.
- Final article output must contain the required number of valid image files (not placeholders).

**Identity consistency mode (when client requires same person):**
- Provide `--reference-image` and `--consistent-person` to `image-gen.js`.
- Prompt must include fixed identity lock constraints: same body/hair/identity, preserve face doodle mask.
- For Doubao image-to-image, enforce high resolution (>= 3686400 px equivalent), and keep face mask style unchanged.
- If provider fallback occurs, report whether consistency was preserved (`source=doubao` preferred).
