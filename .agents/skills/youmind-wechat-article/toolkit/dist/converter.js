...
import { generateTheme, } from './theme-engine.js';
export class WeChatConverter {
     ...
     */
    fixCjkEmphasis(text) {
        return text.replace(/([\u3000-\u303F\uFF01-\uFF60\u2018-\u201F\u2026\u2014\u2013\u00B7])(\*{1,2}|_{1,2})([\u4E00-\u9FFF\u3400-\u4DBF\uF900-\uFAFF])/g, '$1$2\u200A$3');
    }
    stripH1(text) {
        return text
            .split('\n')
    ...
    convertTaskListCheckboxes($) {
        ...
        $('.task-list-item-checkbox').each((_, elem) => {
            ...
            if (isChecked) {
                $(elem).replaceWith(`<span style="display: inline-block; width: 16px; height: 16px; margin-right: 8px; background-color: ${accentColor}; color: #ffffff; border-radius: 3px; text-align: center; line-height: 16px; font-size: 13px; font-weight: bold; vertical-align: middle;">&#10003;</span>`);
            }
            else {
                $(elem).replaceWith(`<span style="display: inline-block; width: 16px; height: 16px; margin-right: 8px; background-color: #ffffff; border: 1.5px solid #d1d5db; border-radius: 3px; text-align: center; line-height: 16px; font-size: 13px; vertical-align: middle;">&nbsp;</span>`);
            }
        });
    }
    applyInlineStyles($) {
        ...
        for (const [tag, style] of Object.entries(tagMap)) {
            ...
            $(tag).each((_, elem) => {
                // 代码块内的 code 单独处理
                if (tag === 'code' && $(elem).parent('pre').length)
                    return;
                // 代码块内的 p/span 等不应用全局样式，避免覆盖代码高亮
                if ($(elem).closest('pre[data-codeblock]').length)
                    return;
    ...
    applyWeChatFixes($) {
        ...
        $('p').each((_, p) => {
            if ($(p).closest('pre[data-codeblock]').length)
                return;
    ...
    generateDigest(html, maxBytes = 120) {
        const $ = cheerio.load(html);
 ...
 */
export function previewHtml(bodyHtml, theme) {
    return `<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
