 ...
 */
export function renderMermaidToPng(code) {
    if (!checkMmdc())
        return null;
    ...
    try {
        ...
        execSync(`mmdc -i "${inputFile}" -o "${outputFile}" -b white --quiet -s 2`, { timeout: 30000, stdio: 'pipe' });
 ...
 */
export function processMermaidBlocks($) {
    if (!checkMmdc()) {
        const hasMermaid = $('pre code.language-mermaid').length > 0;
        if (hasMermaid) {
            console.warn('Found mermaid code blocks but mmdc is not installed.\n' +
    ...
    $('pre').each((_, pre) => {
        ...
        if (pngPath) {
            $(pre).replaceWith(`<p style="text-align: center; margin: 24px 0;"><img src="${pngPath}" alt="Mermaid Diagram" style="max-width: 100%; height: auto;"></p>`);
        }
        // 渲染失败时保留原代码块
    });
}
