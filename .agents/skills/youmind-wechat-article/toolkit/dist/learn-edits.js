...
function analyzeDiff(draftText, finalText) {
    ...
    for (const line of draftLines) {
        const trimmed = line.trim();
        if (trimmed && !finalSet.has(trimmed))
...
function main() {
    ...
    if (!existsSync(draftPath)) {
        console.error(`Draft 文件不存在: ${draftPath}`);
        process.exit(1);
    }
    if (!existsSync(finalPath)) {
        console.error(`Final 文件不存在: ${finalPath}`);
        process.exit(1);
    }
    ...
    if (lessonCount >= 5 && lessonCount % 5 === 0) {
        console.error(`${lessonCount} lessons accumulated. Consider updating playbook:\n` +
