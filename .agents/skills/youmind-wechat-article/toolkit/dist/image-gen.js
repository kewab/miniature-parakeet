...
function buildConsistentPrompt(modules, extraPrompt) {
...
function deriveArticleIdFromOutput(outputPath) {
    const base = basename(outputPath).replace(/\.[^.]+$/, '');
    return base.replace(/-\d+$/, '');
}
function readImageHistory(historyFile) {
    try {
...
function writeImageHistory(historyFile, records) {
    mkdirSync(dirname(historyFile), { recursive: true });
    writeFileSync(historyFile, JSON.stringify(records.slice(-500), null, 2), 'utf-8');
}
function pickNonRecentModules(opts, history, now = Date.now()) {
    ...
    for (let i = 0; i < 30; i++) {
        ...
        if (!recentSet.has(sig))
            return candidate;
    }
    return buildPromptModules(opts);
}
function deriveClientRootFromOutput(outputPath) {
...
function defaultHistoryFile(outputPath) {
    const clientRoot = deriveClientRootFromOutput(outputPath);
    if (clientRoot)
        return `${clientRoot}/history_images.json`;
    return resolve(dirname(outputPath), 'history_images.json');
}
function chooseStockImageIfOldSimilar(history, signature, stockDir, now = Date.now()) {
    const monthMs = 30 * 24 * 3600 * 1000;
...
function isValidImageBuffer(buf, minBytes = 1024) {
    return buf.length >= minBytes && !!detectImageMime(buf);
}
async function downloadImageWithValidation(url, outputPath, opts) {
    try {
...
function toDoubaoImageValue(imageInput) {
...
async function generateDoubao(prompt, apiKey, size, model = 'doubao-seedream-5-0-260128', baseUrl = 'https://ark.cn-beijing.volces.com/api/v3', imageInput) {
    const url = `${baseUrl}/images/generations`;
    const basePayload = {
        model,
        prompt,
...
async function generatePollinations(prompt, outputPath, size = 'article') {
...
async function main() {
    ...
    const usedPosesInArticle = new Set(imageHistory
    ...
    if (args.search) {
        const selectedModules = useConsistency
            ...
            : undefined;
        if (selectedModules) {
            const stock = chooseStockImageIfOldSimilar(imageHistory, modulesSignature(selectedModules), args.stockDir || resolve(dirname(args.output), 'stock'));
            if (stock) {
    ...
    try {
        const imageBytes = providerName === 'doubao' && args.referenceImage
            ? await generateDoubao(finalPrompt, providerCfg.api_key, sizeVal, providerCfg.model, providerCfg.base_url, args.referenceImage)
            : await genFn(finalPrompt, providerCfg.api_key, sizeVal, providerCfg.model, providerCfg.base_url);
