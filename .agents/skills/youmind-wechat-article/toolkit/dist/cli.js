...
import { DEFAULT_COLOR, DEFAULT_THEME, listPresetColors, listThemes, } from './theme-engine.js';
...
function loadCustomTheme(jsonPath) {
    const raw = JSON.parse(readFileSync(resolve(jsonPath), 'utf-8'));
    const styles = raw.styles ?? raw;
    return {
...
program
    ...
    const themeKey = (opts.theme || cfg.theme || DEFAULT_THEME);
    const color = opts.color || cfg.theme_color || DEFAULT_COLOR;
