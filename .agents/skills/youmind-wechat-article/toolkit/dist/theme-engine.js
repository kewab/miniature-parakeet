...
// --- Font Configs ---
const FONTS = {
    default: 'PingFang SC,system-ui,-apple-system,BlinkMacSystemFont,Helvetica Neue,Hiragino Sans GB,Microsoft YaHei UI,Microsoft YaHei,Arial,sans-serif',
...
const HEADING_SIZE_SCALE = {
    minus2: 0.64,
    minus1: 0.8,
    standard: 1.0,
    plus1: 1.15,
};
const HEADING_SPACING_SCALE = {
    minus2: 0.5,
    minus1: 0.72,
    standard: 1.0,
    plus1: 1.18,
...
export class ColorPalette {
    ...
    adjustBrightness(hex, amount) {
        ...
        const adj = (v) => Math.min(255, Math.max(0, Math.round(v * (1 + factor))));
    ...
    mix(c1, c2, ratio) {
        ...
        return ColorPalette.rgbToHex(Math.round(rgb1.r * ratio + rgb2.r * (1 - ratio)), Math.round(rgb1.g * ratio + rgb2.g * (1 - ratio)), Math.round(rgb1.b * ratio + rgb2.b * (1 - ratio)));
    }
    get primary() {
        return this.mainColor;
    }
    get primaryDark() {
        if (this.luminance < 0.4)
            return this.mix(this.mainColor, '#ffffff', 0.85);
        return this.adjustBrightness(this.mainColor, -0.2);
    }
    get primaryLight() {
        if (this.luminance < 0.4)
            return this.mix(this.mainColor, '#ffffff', 0.6);
        return this.adjustBrightness(this.mainColor, 0.2);
    }
    get primaryLightest() {
...
function containerStyle(ff, fs = 16) {
...
function pLayout(fs = 16, ps = 'normal') {
...
function genSimple(p, ff, fs, ps, hs) {
    const fv = getFontVal(ff);
...
function genCenter(p, ff, fs, ps, hs) {
    const fv = getFontVal(ff);
    const font = `font-family: ${fv};`;
    const pl = pLayout(fs, ps);
    return {
...
function genDecoration(p, ff, fs, ps, hs) {
    const fv = getFontVal(ff);
    const font = `font-family: ${fv};`;
    const pl = pLayout(fs, ps);
    return {
...
function genProminent(p, ff, fs, ps, hs) {
    const fv = getFontVal(ff);
    const font = `font-family: ${fv};`;
    const pl = pLayout(fs, ps);
    return {
...
export function generateTheme(options = {}) {
    const { themeKey = 'simple', color = DEFAULT_COLOR, fontFamily = 'default', fontSize = 16, headingSize = 'standard', paragraphSpacing = 'normal', } = options;
    const key = (themeKey in THEME_GENERATORS ? themeKey : 'simple');
...
export function listPresetColors() {
